...
/* Comments beginning with //gAt// are denoted as being written
 * by the Google Android team. Example:
 * 
 * //gAt//candy corn
 */

/* Comments beginning with //tL// are denoted as being written
 * by thoriumLabs. Example:
 * 
 * //tL//should be 0.25
 /*/
...
import com.thoriumLabs.android.dessertcase.R;
...
public class DessertCaseView extends FrameLayout {
    ...
    //tL//delay from launch to first generated animations
    //tL//default is 5000(ms)
    static int START_DELAY;
    //tL//delay between generated animations
    //tL//default is 2000(ms)
    static int DELAY;
    //tL//duration of animations, both generated and user-invoked
    //tL//default is 500(ms)
    static int DURATION;
    //tL//maximum hue for generation
    //tL//default is 12(?)
    static int COLORS;
    ...
    private static final int[] PASTRIES = {
            R.drawable.dessert_kitkat,      //gAt//used with permission
            R.drawable.dessert_android,     //gAt//thx irina
    };

    private static final int[] RARE_PASTRIES = {
            R.drawable.dessert_cupcake,     //gAt// 2009
            R.drawable.dessert_donut,       //gAt// 2009
            R.drawable.dessert_eclair,      //gAt// 2009
            R.drawable.dessert_froyo,       //gAt// 2010
            R.drawable.dessert_gingerbread, //gAt// 2010
            R.drawable.dessert_honeycomb,   //gAt// 2011
            R.drawable.dessert_ics,         //gAt// 2011
            R.drawable.dessert_jellybean,   //gAt// 2012
    };

    private static final int[] XRARE_PASTRIES = {
            R.drawable.dessert_petitfour,   //gAt// the original and still delicious

            R.drawable.dessert_donutburger, //gAt// remember kids, this was long before cronuts

            R.drawable.dessert_flan,        //gAt//     sholes final approach
                                            //gAt//     landing gear punted to flan
                                            //gAt//    runway foam glistens
                                            //gAt//         -- mcleron

            R.drawable.dessert_keylimepie,  //gAt// from an alternative timeline
    };
    private static final int[] XXRARE_PASTRIES = {
            R.drawable.dessert_zombiegingerbread, //gAt// thx hackbod
            R.drawable.dessert_dandroid,    //gAt// thx morrildl
            R.drawable.dessert_jandycane,   //gAt// thx nes
    };
    ...
    public static final float SCALE = 0.25f; //tL//should be 0.25 //gAt//natural display size will be SCALE*mCellSize
    ...
    private final Runnable mJuggle = new Runnable() {
        ...
        public void run() {
            ...
            final int K = 1; //gAt//irand(1,3);
            for (int i=0; i<K; i++) {
                final View child = getChildAt((int) (Math.random() * N));
                place(child, do_animate);
            }
    ...
    public DessertCaseView(Context context) {
        ...
        is_interactive = prefs.getBoolean("enable_interactivity",true);
        do_animate = prefs.getBoolean("enable_animation",true);
        show_overlay = prefs.getBoolean("enable_overlay",true);
        START_DELAY = Integer.valueOf(prefs.getString("animation_start_delay", "5000"));
        DURATION = Integer.valueOf(prefs.getString("animation_duration", "500"));
        DELAY = Integer.valueOf(prefs.getString("animation_delay", "2000"));
        COLORS = Integer.valueOf(prefs.getInt("hue_maximum", 12));
    }
    ...
    public DessertCaseView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (mCellSize < 512) { //gAt// assuming 512x512 images
            opts.inSampleSize = 2;
        }
    ...
    int random_color() {
    	//gAt//  return 0xFF000000 | (int) (Math.random() * (float) 0xFFFFFF); // totally random
...
if(show_overlay){
            ...
            addView(v, lp);
            /*//tL//
             * This is disabled because it makes a weird animation
             * where the empty tiles are filled in at an angle instead
             * of coming out of the center
            */
            place(v, pt, false);
            if (animationLen > 0) {
